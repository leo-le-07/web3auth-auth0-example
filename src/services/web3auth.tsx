...
export const Web3AuthProvider: FunctionComponent<IWeb3AuthState> = ({ children, web3AuthNetwork, chain }: IWeb3AuthProps) => {
  ...
  const { logout: logoutAuth, getIdTokenClaims, isAuthenticated } = useAuth0();
  ...
  useEffect(() => {
    ...
    async function init() {
      try {
        setIsLoading(true);
        const clientId = "BACx5mWQ0ujIQI95iU418ikJ8qbipUC3idPKDR5zDBzOf-gMh97VRDu4ApZqXSXXWtM74LLJfX6HHJUTYi9XWhA";
  ...
  const logout = async () => {
    ...
    setProvider(null);
    logoutAuth({ returnTo: "http://localhost:3000/login" })
  };
